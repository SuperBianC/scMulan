...
def GeneSymbolUniform(input_adata=None, input_path=None, ref_table_path=None, 
    ...
    logger.info(f"The shape of query data is: {query_data.shape}")
    ...
    logger.info(f"The length of reference gene_list is: {len(total_gene_list)}")
    ...
    if n_row >= 1e5:
        logger.info(f"Because the number of cells is very large, we construct the sparse matrix parallelly with {n_threads} threads")
    ...
    logger.info(f"Shape of output data is {result_data.shape}. It should have 42117 genes with cell number unchanged.")
    ...
    logger.info(f"h5ad file saved in:{os.path.join(output_dir,f'{output_prefix}_uniformed.h5ad')}" )
    
    if print_report:
        report.to_csv(os.path.join(output_dir,"{}_report.csv".format(output_prefix)))
        logger.info(f"report file saved in: {os.path.join(output_dir,f'{output_prefix}_report.csv')}")
