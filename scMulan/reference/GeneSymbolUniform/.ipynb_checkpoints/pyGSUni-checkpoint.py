...
def GeneSymbolUniform(input_adata=None, input_path=None, ref_table_path="./GeneSymbolRef_upd0731.csv", 
    ...
    if input_path is None and input_adata is None:
        ...
        logger.error("Error: No input data is given.")
        return(-1)
    ...
    logger.info("The shape of query data is: {} ".format(query_data.shape))
    ...
    logger.info("The length of reference gene_list is: {}".format(len(total_gene_list)))

    # *--------------------Performing Gene Symbol Uniform--------------------*
    logger.info("Performing gene symbol uniform, this step may take several minutes")
    ...
    # *--------------------Build output data--------------------*
    logger.info("Building output data, this step may take several minutes")
    ...
    if n_row >= 1e5:
        logger.info("Because the number of cells is very large, we construct the sparse matrix parallelly with {} threads".format(n_threads))
    ...
    logger.info("Shape of output data is {}. It should have 42117 genes with cell number unchanged.".format(result_data.shape))
    
     # *--------------------Build output data--------------------*
    logger.info("=========Building output h5ad=========")
    adata_output = sc.AnnData(X=result_data,obs=adata_input.obs,var=pd.DataFrame(index=total_gene_list))

    logger.info("=========Saving output h5ad=========")
    adata_output.write(os.path.join(output_dir,"{}_uniformed.h5ad".format(output_prefix)))
    logger.info("h5ad file saved in:", os.path.join(output_dir,"{}_uniformed.h5ad".format(output_prefix)))
    
    if print_report:
        logger.info("=========Printing report=========")
        report.to_csv(os.path.join(output_dir,"{}_report.csv".format(output_prefix)))
        logger.info("report file saved in:", os.path.join(output_dir,"{}_report.csv".format(output_prefix)))
